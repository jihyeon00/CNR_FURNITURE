...
$(document).ready(function() {
	...
	$('#qsDiv1').on('change', function() {
		...
		if (qsDiv1) {
			$.ajax({
				url: '/qsDiv2ListOB?qsDiv1=' + qsDiv1,
				type: 'GET',
				dataType: 'json',
	...
	$('#qsDiv1ForInsertModal').on('change', function() {
	  ...
	  if (qsDiv1) {
	    $.ajax({
	      url: '/qsDiv2OBModalList?qsDiv1=' + qsDiv1,
	      type: 'GET',
	      dataType: 'json',
	      success: function(data) {
	        if (data.length > 0) {
	...
	/** [등록] 모달창 
	 * - [작업번호] 입력에 따른 [제조LOT번호], [공정번호], 
	 * [제품번호], [제품명], [제품규격], [단위], [일일총생산수량] 자동 채우기
	 */
	$('#workIDForInsertModal').change(function() {
		var workID = $(this).val();
		
		$.ajax({
			url: '/workDetailsForOBInsertModal',
			type: 'GET',
			data: {workID: workID},
			
			success: function(data) {	// 성공 시
				$('#lotIDForInsertModal').val(data.lotID);													// 제조LOT번호
				$('#processIDForInsertModal').val(data.processID);									// 공정번호
				$('#prodIDForInsertModal').val(data.productID);											// 제품번호
				$('#prodNameForInsertModal').val(data.productName);									// 제품명
				$('#prodColorForInsertModal').val(data.productColor);									// 제품색상
				$('#prodStandardForInsertModal').val(data.productStandard);					// 제품규격
	...
	$('#addInspectionBtn').click(function() {
		...
		// 모달창에서 입력된 데이터 수집
		var data = collectDataFromModal();	// 제품불량데이터
	...
	function collectDataFromModal() {
		...
		return {
			...
			// 2행: 제조LOT번호, 공정번호, 제품번호
			lotID: $('#lotIDForInsertModal').val(),
			processID: $('#processIDForInsertModal').val(),
			productID: $('#prodIDForInsertModal').val(),
			// 3행: 제품명, 제품색상, 제품규격
			productName: $('#prodNameForInsertModal').val(),
			productColor: $('#prodColorForInsertModal').val(),
			productStandard: $('#prodStandardForInsertModal').val(),
			// 3행: 단위, 일일총작업수량, 검사수량
	...
	function addToInspectionList(data) {
		...
		var newRow = `<tr>
			...
			<td class="productColor">${data.productColor}</td>
			<td class="productStandard">${data.productStandard}</td>
    ...
    $('#registerOBInspection input, #registerOBInspection select').val('');
	}
	...
	$('#registerBtn').click(function() {
		...
		if(confirm('제품불량정보를 등록하시겠습니까?')) {
			console.log(items);
			registerInspectionOB(items);	// 수집된 데이터를 DB에 저장
		}
	});
	...
	function collectItemsFromTable() {
		...
		$('.newInspList tbody tr').each(function() {
			...
			items.push({
				...
				productColor: row.find('.productColor').text(),
				productStandard: row.find('.productStandard').text(),
	 ...
	 * - 데이터베이스 저장 로직
	 */
	function registerInspectionOB(items) {
		$.ajax({
			url: '/inspectionOB/register',	// 데이터 저장을 처리하는 서버의 API 경로
			type: 'POST',
	...
	$(document).on('click', '.editBtn', function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$('#editProductColor').val(data.productColor);
				$('#editProductStandard').val(data.productStandard);
	...
	function updateQsDiv2Options() {
		...
		$.ajax({
			url: '/qsDiv2ListForOBEdit?qsDiv1=' + qsDiv1,
			type: 'GET',
			dataType: 'json', 
			success: function(data) {
	...
	$('#updateBtn').on('click', function() {
		...
		$.ajax({
			url: '/inspectionOB/update',	// 수정 처리를 위한 서버 URL
			...
			success: function(response) {
				if(response.success) {
					alert('수정이 완료되었습니다.');
					window.location.href = '/M/quality/inspectionOB';	// 성공 후, '/inspectionProcess' 페이지로 리다이렉트
				} else {
